...
from MusicXMLPaser import StaticAssets

import fractions
import xml.etree.ElementTree as ElementTrees
import zipfile
...
class MusicXMLDocument(object):
    ...
    def __init__(self, filename):
        ...
        self._score_parts = {}
        self.midi_resolution = StaticAssets.STANDARD_PPQ
    ...
    def _get_score(filename):
        ...
        if filename.endswith('.mxl'):
            ...
            if container_file:
                try:
                    container = ElementTrees.fromstring(mxlzip.read(container_file[0]))
                    for rootfile_tag in container.findall('./rootfiles/rootfile'):
                        if 'media-type' in rootfile_tag.attrib:
                            ...
                            if not compressed_file_name:
                                ...
                                raise MusicXMLParseError(
                                    'Multiple MusicXML files found in compressed archive')
                except ElementTrees.ParseError as exception:
                    raise MusicXMLParseError(exception)
            ...
            try:
                score = ElementTrees.fromstring(score_string)
            except ElementTrees.ParseError as exception:
                raise MusicXMLParseError(exception)
        else:
            # Uncompressed XML file.
            try:
                tree = ElementTrees.parse(filename)
                score = tree.getroot()
            except ElementTrees.ParseError as exception:
                raise MusicXMLParseError(exception)
...
def _repair_empty_measure(measure):
    ...
    if note_count == 0 and forward_count == 1:
        ...
        new_note += '</note>'
        new_note_xml = ElementTrees.fromstring(new_note)
...
class Measure(object):
    ...
    def _parse_backup(self, xml_backup):
        ...
        midi_ticks = backup_duration * (StaticAssets.STANDARD_PPQ
                                        / self.state.divisions)
        seconds = ((midi_ticks / StaticAssets.STANDARD_PPQ)
    ...
    def _parse_forward(self, xml_forward):
        ...
        xml_duration = xml_forward.find('duration')
        forward_duration = int(xml_duration.text)
        midi_ticks = forward_duration * (StaticAssets.STANDARD_PPQ
                                         / self.state.divisions)
        seconds = ((midi_ticks / StaticAssets.STANDARD_PPQ)
                   * self.state.seconds_per_quarter)
        self.state.time_position += seconds
...
def _alter_to_string(alter_text):
...
def _parse_degree(xml_degree):
    ...
    if xml_degree.find('degree-alter') is not None:
        ...
        alter_string = _alter_to_string(alter_text)
    ...
    def _parse(self):
        ...
        for child in self.xml_harmony:
            if child.tag == 'root':
                ...
                self.degrees.append(_parse_degree(child))
            elif child.tag == 'bass':
                ...
                midi_ticks = offset * StaticAssets.STANDARD_PPQ / self.state.divisions
                seconds = (midi_ticks / StaticAssets.STANDARD_PPQ *
                           self.state.seconds_per_quarter)
...
class Tempo(object):
    ...
    def _parse(self):
        ...
        if self.qpm == 0:
            ...
            self.qpm = StaticAssets.DEFAULT_QUARTERS_PER_MINUTE
        self.time_position = self.state.time_position
